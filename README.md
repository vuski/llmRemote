...
![LlmRemote](image/image.png)

- 원격에서 휴대폰 브라우저로 윈도우 PC의 특정 창을 선택하여 화면을 보고, 텍스트를 입력할 수 있는 원격 접속 앱입니다.
- cursor에 익스텐션으로 붙여둔 클로드코드 사용중이며, 자리를 떠나도 작업을 확인하고 계속 진행시키려는 목적으로 만들었습니다.
- 클로드 코드 창을 커서에서 독립시켜 새 윈도우 창으로 떼어놓아야 조작하기 편리합니다.
- 개인적으로 딱 필요한 정도까지만 만들어서 많이 허술합니다. 그래도 아주 기본적 보안은 챙겼고, 의외로 쏠쏠하게 쓸만합니다.
- 당연히 커서 외에도 다른 앱들도 사용가능하지만, 기본적으로 창을 작게 만들어 접근하는 방식이라, 글자가 작아지거나 메뉴 비중이 커지면 다루기 어렵습니다.
- 전체 앱의 99.9%를 클로드 코드를 이용하여 만들었습니다.
